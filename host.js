...
return {
  ...
  apply(ctx) {
    ...
    console.log('mcmp: services available -> commands=' + !!commands + ' fs=' + !!fs + ' agents=' + !!agents + ' subagents=' + !!subagents + ' systemPrompt=' + !!systemPrompt)
    ...
    function buildPrompt(round, si, ii, s, it, args) {
      ...
      if (args.problem.kind === 'file') {
        ...
        if (si === 0 && ii === 0) p.push('【先存档】请先把上面嵌入的赛题原文完整保存为 ' + args.problemPath + ' 文件,供后续所有步骤读取。')
      }
      p.push('【必读文件】')
      p.push('- ' + s.file + ' (本步骤历次迭代的成果,本次要在此基础上改进)')
      for (let q = 0; q < si; q++) p.push('- ' + STEPS[q].file + ' (前序步骤的权威成果,必须严格遵守其结论与符号体系)')
      if (round > 1) {
        p.push('上一轮(第 ' + (round - 1) + ' 轮)的全部成果都已保存在上述文件中(标题形如「## 第' + (round - 1) + '轮…」)。你必须先定位并阅读上一轮对应内容,本轮产出必须在上一轮基础上持续优化,质量不得低于上一轮。')
      }
      if (ii === 0) {
        p.push('若本步骤文件尚不存在或为空,从零开始建立该步骤的成果。')
      } else {
        p.push('若文件中缺失上一迭代(「第' + round + '轮·迭代' + ii + ' ' + s.iters[ii - 1].name + '」)的成果,说明上一迭代失败:请先补齐该迭代的成果,再完成本次迭代。')
      }
      p.push('')
      ...
      p.push('')
      p.push('【成果保存(强制)】')
      p.push('1. 将本次迭代的完整成果(包括质疑清单与全部推导、公式、数据、表格、代码、图表说明)追加写入 ' + s.file + ',以标题「## 第' + round + '轮·迭代' + (ii + 1) + ' ' + it.name + '」开头。内容必须完整、可直接复现,不得用「同上」「略」等缩写。')
