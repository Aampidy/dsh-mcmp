...
export function createPipeline({ fs, subagents, agents, retryBackoffMs }) {
        ...
        if (decision === 'ROLLBACK') {
          ...
          if ((stageCounts[targetIdx] || 0) >= ROLLBACK_LIMIT) {
            ...
            if (stageCounts[targetIdx] >= ROLLBACK_LIMIT) {
              // 防崩溃(架构 3.7):同一阶段回滚≥10 次 → FORCED_FINAL 强制锁定,通知人工,问题降级为 P3 处理,继续推进
  ...
  function start({ session, agent, args, resumeCount, explicitModel }) {
    ...
    pushLog('流水线已启动: ' + args.rounds + ' 遍 × 五阶段 ' + PER_ROUND + ' 个子阶段(中控台双重质疑:扫描验证 + 评审决策),输出目录 ' + args.wsDir + (resumeCount > 0 ? '(断点续跑:从第 ' + (resumeCount + 1) + ' 个子阶段继续)' : '') + '(关闭本聊天窗口不影响后台执行)')
