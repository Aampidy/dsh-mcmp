/**
 * dsh-mcmp · Client 半(标准 __ModuleLoader__ 客户端 bundle)
 * 浮动进度面板:每 1.2s 通过 fetch('/mcmp-api/state') 轮询 Host 状态,
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      if (!minimized) {
        ...
        if (st === 'running') {
          ...
          actions.push(h('button', { key: 'reset', className: 'mcmp-btn', onClick: () => { fetch('/mcmp-api/reset', { method: 'POST' }).then(() => setDismissedRun(snap.runId)).catch(() => {}) } }, '清空状态'))
        }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        { name: 'shell.overlay', id: 'mcmp-pipeline-panel', order: 50, label: '数学建模流水线进度面板' },
        () => h(Panel),
      ))
    }
