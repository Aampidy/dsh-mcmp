...
export function apply(ctx) {
  ...
  console.log('dsh-mcmp: services available -> commands=' + !!commands + ' fs=' + !!fs + ' agents=' + !!agents + ' subagents=' + !!subagents + ' systemPrompt=' + !!systemPrompt + ' webServer=' + !!webServer + ' timer=' + !!timerSvc)

  const FLOW_NAME = '数学建模论文流水线v2'
  // 全流水线缺陷台账:跨步骤错误登记、裁决与向上修复的唯一权威文件
  const LEDGER = '00_缺陷与裁决台账.md'

  // 迭代级重试:一次迭代失败(如模型 429/5xx 瞬时抖动)后自动退避重试,
  // 避免"连续 3 个子任务失败"被瞬时故障触发。重试间隔可用 --retry-ms=N 覆盖(测试/调优用)。
  const DEFAULT_RETRY_DELAYS = [15000, 45000]
  const MAX_ITER_RETRY = 2
  ...
  function sleepAbortable(ms, signal) {
    return new Promise((resolve) => {
      ...
      const finish = () => {
        if (settled) return
        settled = true
        if (timerDispose) { try { timerDispose() } catch (err) { /* ignore */ } }
        resolve()
      }
      if (signal && signal.aborted) { resolve(); return }
      if (signal && typeof signal.addEventListener === 'function') signal.addEventListener('abort', finish, { once: true })
      if (timerSvc && typeof timerSvc.timeout === 'function') {
        try { timerDispose = timerSvc.timeout(finish, ms) } catch (err) { finish() }
      } else {
        finish() // 无 timer 服务时不做等待,直接重试
      }
    })
  }
  ...
  let starting = false // 启动锁:async startPipeline 中等待目录扫描时防止并发启动
  ...
  async function listMdFiles(dir) {
    ...
    try {
      ...
      return entries
        ...
        .map((e) => e.name)
        .sort()
    } catch (err) { /* 目录不存在/不可读时返回空 */ }
    return []
  }

  function pickMdMatch(names, partial) {
    const p = (partial || '').toLowerCase()
    const exact = names.filter((n) => n.toLowerCase() === p)
    if (exact.length === 1) return { kind: 'match', name: exact[0] }
    const starts = names.filter((n) => n.toLowerCase().startsWith(p))
    if (starts.length === 1) return { kind: 'match', name: starts[0] }
    if (starts.length > 1) return { kind: 'ambiguous', names: starts }
    const contains = names.filter((n) => n.toLowerCase().indexOf(p) >= 0)
    if (contains.length === 1) return { kind: 'match', name: contains[0] }
    if (contains.length > 1) return { kind: 'ambiguous', names: contains }
    return { kind: 'none' }
  }

  async function resolveFromPath(cwd, raw) {
    // 返回 { kind:'ok', path } | { kind:'error', text }
    let p = (raw || '').trim().replace(/^["']|["']$/g, '')
    const cwdNorm = cwd.replace(/[\\/]+$/, '')
    if (p.length === 0) {
      const names = await listMdFiles(cwd)
      if (names.length === 0) return { kind: 'error', text: '--from 未指定文件,且工作区根目录下没有 .md 文档。' }
      return { kind: 'error', text: '--from 未指定文件。工作区根目录下的 MD 文档:\n' + names.map((n) => '  - ' + n).join('\n') + '\n请用 --from 文件名(支持部分匹配,如 --from 2024)重新指定。' }
    }
    // 绝对路径
    if (/^[A-Za-z]:[\\/]/.test(p) || p.indexOf('/') === 0 || p.indexOf('\\') === 0) {
      return { kind: 'ok', path: p }
    }
    // 含路径分隔符的相对路径:直接按路径解析并验证存在
    if (p.indexOf('/') >= 0 || p.indexOf('\\') >= 0) {
      const full = cwdNorm + '/' + p
      try {
        const t = await fs.resolve(full)
        const st = await fs.stat(t)
        if (st && st.type === 'file') return { kind: 'ok', path: full }
        return { kind: 'error', text: '路径不是文件: ' + p }
      } catch (err) { /* fallthrough 到模糊匹配 */ }
    }
    // 模糊匹配文件名
    const names = await listMdFiles(cwd)
    const pick = pickMdMatch(names, p)
    if (pick.kind === 'match') return { kind: 'ok', path: cwdNorm + '/' + pick.name }
    if (pick.kind === 'ambiguous') {
      return { kind: 'error', text: '--from "' + p + '" 匹配到多个文档,请补充关键字:\n' + pick.names.map((n) => '  - ' + n).join('\n') }
    }
    const hint = names.length > 0 ? '\n工作区根目录现有 MD 文档:\n' + names.map((n) => '  - ' + n).join('\n') : ''
    return { kind: 'error', text: '未找到匹配 "' + p + '" 的 MD 文档。' + hint }
  }
  ...
  async function runPipeline(runId, session, agent, args, controller) {
    ...
    for (let g = args.startIndex; g < total; g++) {
      ...
      let outcome = 'failed'
      let note = ''
      // 迭代级重试:失败(如模型 429/5xx 瞬时抖动)后自动退避重试,吸收瞬时故障
      const retryDelays = Array.isArray(args.retryDelays) && args.retryDelays.length ? args.retryDelays : DEFAULT_RETRY_DELAYS
      let res = null
      for (let attempt = 0; ; attempt++) {
        res = await runChild(provider, session, agent, controller, label, buildPrompt(r, f.s, f.i, s, it, args), (child) => {
          if (!started) {
            started = true
            appendRec(runId, session, 'tool-workflow/agent-start', { runId, seq: g + 1, label, phase: s.phase, childId: String(child.id) })
          }
        })
        if (res.outcome !== 'failed' || attempt >= MAX_ITER_RETRY) break
        const delayMs = retryDelays[Math.min(attempt, retryDelays.length - 1)] || 0
        pushLog('迭代失败,将在 ' + Math.round(delayMs / 1000) + 's 后重试(第 ' + (attempt + 1) + '/' + MAX_ITER_RETRY + ' 次): ' + label)
      ...
      if (failStreak >= 3) {
        stopReason = 'error'
        errorMsg = '连续 3 个子任务失败(每个失败迭代已含自动重试),流水线提前终止。常见原因:模型服务限流(429)/账户余额不足(402)/上下文过长/工作区文件问题;请排查后重新 /loopbegin 续跑,或 /loopreset 后重来。已有成果全部保留。'
        break
      }
    }
  ...
  // ---------- 共享的流水线启动逻辑(async:--from 需要异步扫描目录;启动锁防并发) ----------
  async function startPipeline(agent, rawInput) {
    if (state.status === 'running' || starting) {
    ...
    try {
      ...
      // 迭代失败重试间隔(毫秒),默认 [15s, 45s];--retry-ms=N 可覆盖(测试/调优)
      const mRetry = /(?:^|\s)--retry-ms\s*[=:]?\s*(\d+)/i.exec(raw)
      const retryDelays = mRetry ? [Math.max(1, parseInt(mRetry[1], 10)), Math.max(1, parseInt(mRetry[1], 10))] : DEFAULT_RETRY_DELAYS
      // --from:允许只写 --from(列出根目录 MD 文档)或部分文件名(自动匹配补全)
      const hasFrom = /(?:^|\s)--from\b/i.test(raw)
      const mFrom = hasFrom ? /(?:^|\s)--from(?:=)?\s*([^\r\n]+?)(?=\s+--|\r?$)/i.exec(raw) : null
      let problem = null
      if (hasFrom) {
        const resolved = await resolveFromPath(cwd, mFrom ? mFrom[1] : '')
        if (resolved.kind === 'error') return { kind: 'error', text: resolved.text }
        problem = { kind: 'file', path: resolved.path }
      } else {
        const text = extractProblem(session)
        if (text.trim().length < 40) {
          return { kind: 'error', text: '未能从对话中提取到赛题。请先把完整赛题粘贴到对话框并发送,再发送以 /loopbegin 开头的消息;或使用 /loopbegin --from 文件名(支持部分匹配自动查找根目录 .md 文档)。' }
        }
      ...
      const args = { rounds, wsDir, problemPath, problem, title: title ? title.slice(0, 40) : '赛题', vision, retryDelays }
  ...
  if (commands) {
    try {
      commands.register({
        ...
        input: { hint: '--round=N 外循环轮数(默认 1);--from 文件名(支持部分匹配,自动查找根目录 .md 文档,如 --from 2024);--fresh 从头重跑' },
  ...
  ctx.on('session/event', (session, event) => {
    try {
      ...
      Promise.resolve(startPipeline(agent, firstLine)).then((res) => {
