...
export function apply(ctx) {
  ...
  function startPipeline(agent, rawInput) {
    try {
      ...
      // --model 提供商/模型:强制所有流水线子任务使用指定模型路由
      // (否则子任务继承父会话创建时固化的模型选项,聊天里的模型切换对子任务不生效)
      let agentOptions = undefined
      {
        ...
        if (mModel) {
          ...
          if (slash >= 0) {
            agentOptions = { provider: rawModel.slice(0, slash), model: rawModel.slice(slash + 1) }
          } else if (mProvider) {
            agentOptions = { provider: mProvider[1], model: rawModel }
          } else {
      ...
      if (agentOptions) pushLog('子任务模型路由:强制 ' + agentOptions.provider + '/' + agentOptions.model + '(--model)')
      refreshFiles()
